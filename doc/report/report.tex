...
Neben dem regulären Import existiert auch noch der unqualifizierte Import. Dieser ist für den Programmierer unzugänglich. Er dient zum Einbinden des Preludes, sodass Funktionen wie Addition auch ohne Modulbezeichner verwendet werden können. Das Prelude wird implizit von jedem Modul importiert. Der unqualifizierte Import ist von der Prüfung von doppelten Modulbezeichnern ausgeschlossen, jedoch nicht von Tests auf Pfade.
...
Exportiert werden nur jene Funktionen und Datentypen, welche durch das Schlüsselwort \verb|PUBLIC| als öffentlich deklariert wurden. Dies erzwingt die Angabe einer Signatur für Funktionen, die exportiert werden sollen. Der Typ einer öffentlichen Funktion kann also nicht inferiert werden. Dadurch ist gewährleistet, dass die Schnittstellen des Moduls immer so sind, wie der Programmierer es erwartet. Für uns war allerdings entscheidend, dass wir auf inferierte Typen keinen leichten Zugriff haben. Das liegt daran, dass die abstrakte SL-Syntax zum Type-Checking in ELC übersetzt wird. Daher können wir nicht mehr ermitteln, welches ELC-Konstrukt zu welchem SL-Konstrukt gehört. Dieses Problem ist zwar lösbar, indem man explizite Verweise in den ELC-Baum angibt, wurde von uns aber nicht weiter verfolgt.
...
gespeichert sind}
\TODO{wenn mehr als ein Modul eine main-Methode bereitstellt, kann nur
die zuletzt ausgeführte verwendet werden}
...
\begin{description}
 ...
 \item[\texttt{cycleA.sl} und \texttt{cycleB.sl}] enhalten eine direkte
